...
pipeline {
  ...
  stages {
    ...
    stage("Prepare Environment") {
      steps {
        ...
        dir("pipeline") {
          ...
          git branch: 'main', credentialsId: 'github', url: 'https://github.com/Josephboadi/joy-jenkins-automation'
          script {
            groovyMethods = load("functions.groovy")
          }
        }
        // Add your chat chat url to url field
        git branch: 'main', credentialsId: 'github', url: 'https://github.com/Josephboadi/joy-jobber-chat'
        sh 'npm install'
      }
    }
    ...
    stage("Create New Pods") {
      steps {
        withKubeCredentials(kubectlCredentials: [[caCertificate: '', clusterName: 'minikube', contextName: 'minikube', credentialsId: 'jenkins-k8s-token', namespace: '', serverUrl: 'https://192.168.105.4:8443']]) {
          script {
